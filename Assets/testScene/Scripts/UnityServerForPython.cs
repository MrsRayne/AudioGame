...
public class UnityServerForPython : MonoBehaviour //if you dont understand whats happening here, neither do we, we asked an IT guy to help us with this part
{
    ...
    void Update()
    {
        if (clientMessage != "") //ExecuteRasaMessage.Instance.ExecuteMessage(clientMessage) cannot be called in a Thread, so this thing here exists
        {
            string message = clientMessage;
            clientMessage = "";
            //ExecuteRasaMessage.Instance.ExecuteMessage(message);
